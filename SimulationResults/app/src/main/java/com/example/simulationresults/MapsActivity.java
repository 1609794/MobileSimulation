...
public class MapsActivity extends FragmentActivity implements OnMapReadyCallback, GoogleMap.OnMarkerClickListener {
    ...
    public void onMapReady(GoogleMap googleMap) {
        ...
        Mali = mMap.addMarker(new MarkerOptions().position(M).title("Mali"));
        LatLng S = new LatLng(7.664594, 29.961614);
        SouthSudan = mMap.addMarker(new MarkerOptions().position(S).title("South Sudan"));
        LatLng C = new LatLng(6.425652, 20.456225);
        CAR = mMap.addMarker(new MarkerOptions().position(C).title("Central African Republic"));
        LatLng B = new LatLng(-3.306623, 29.979213);
        Burundi = mMap.addMarker(new MarkerOptions().position(B).title("Burundi"));
